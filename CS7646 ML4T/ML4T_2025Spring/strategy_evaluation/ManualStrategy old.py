...
class ManualStrategy(object):
    ...
    def testPolicy(self, symbol="AAPL", sd=dt.datetime(2010,1,1), ed=dt.datetime(2011,12,31), sv=100000):
        ...
        sma, \
        bbands, \
        rsi = ManualStrategy._compute_indicators(prices, highs, lows, volumes, 10)

        trading_dates = prices.index

        return self._generate_orders(symbol, trading_dates, (mtm, sma, bbands, rsi))
    ...
    def _should_long(symbol, today, yesterday, indicators, current_shares):
        ...
        # RSI < 30 -> by definition (https://www.investopedia.com/terms/r/rsi.asp)
        ...
        rsi_oversold = rsi.loc[today, symbol] < 30
        bband_crossing_below_and_up = bbands.loc[today, symbol] < -1.

        return rsi_oversold and \
    ...
    def _should_short(symbol, today, yesterday, indicators, current_shares):
        ...
        # 1) RSI > 70 -> by definition (http://www.investopedia.com/terms/r/rsi.asp)
        ...
        rsi_overbought = rsi.loc[today, symbol] > 70
        bband_crossing_above_and_down = bbands.loc[today, symbol] > 1

        return rsi_overbought and \
...
def main():
    ...
    manual_port_vals_in, manual_orders_in, long_short_data_in = _evaluate_manual_strategy(
        symbol="JPM",
        sd=dt.datetime(2008, 1, 1),
        ed=dt.datetime(2009, 12, 31),
        sv=100000
    )
    benchmark_port_vals_in = _evaluate_benchmark(manual_port_vals_in.index)

    # Out-of-sample evaluation
    manual_port_vals_out, manual_orders_out, long_short_data_out = _evaluate_manual_strategy(
        symbol="JPM",
        sd=dt.datetime(2010, 1, 1),
        ed=dt.datetime(2011, 12, 31),
        sv=100000
    )
    benchmark_port_vals_out = _evaluate_benchmark(manual_port_vals_out.index)
    ...
    # manual_orders.to_csv(f'{save_folder}manual_strategy_orders_in_sample.csv')
    # Save the orders generated for debugging purposes

    manual_orders_in.to_csv(f'{save_folder}manual_strategy_orders_in_sample.csv')
    manual_orders_out.to_csv(f'{save_folder}manual_strategy_orders_out_sample.csv')

    # Normalize values
    manual_port_vals_in = manual_port_vals_in / manual_port_vals_in.iloc[0]
    benchmark_port_vals_in = benchmark_port_vals_in / benchmark_port_vals_in.iloc[0]
    manual_port_vals_out = manual_port_vals_out / manual_port_vals_out.iloc[0]
    benchmark_port_vals_out = benchmark_port_vals_out / benchmark_port_vals_out.iloc[0]
...
# def _evaluate_manual_strategy():
def _evaluate_manual_strategy(symbol, sd, ed, sv):
    ms = ManualStrategy()
    orders = ms.testPolicy(symbol=symbol, sd=sd, ed=ed, sv=sv)
    ...
    print(f"\n===== Manual Strategy Stats ({sd.date()} to {ed.date()}) =====")
...
#     # save_path = './CS7646 ML4T/ML4T_2025Spring/strategy_evaluation/images'
#     save_path = './'

#     plt.savefig(f"{save_path}/{title.replace(' ', '')}.png", bbox_inches='tight')

import matplotlib.pyplot as plt
import matplotlib.dates as mdates

def _plot(data, long_short_data, title, xlabel, ylabel, colors=['purple', 'red'], legend=None):
    ...
    # save_folder = './'
    plt.savefig(f"{save_folder}/{title.replace(' ', '')}.png", bbox_inches='tight')
